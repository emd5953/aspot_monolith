...
(Restored 2026-07-11: this file was removed on origin/main in `e857641`,
presumably to keep it out of the public repo — the loop still needs it locally.
Feature backlog is now empty; the loop is in perpetual-audit mode.)
...
- [x] "Tidy this day" — reorder a day's stops by proximity. Backend shipped
      upstream in `47ae79f` (POST /activities/tidy, pure `canTidyDay` gate +
      `nearestNeighborOrder`, 9 tests, owner-only, 409 when not tidy-able).
      Frontend: "Tidy route" button in the spread-out hint, same gate, calls
      the endpoint + refetches, 5 render tests. — `101922b`
